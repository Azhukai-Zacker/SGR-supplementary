...
# Optionally point NLTK to a local data directory.
...
def load_generated_captions(cap_file, image_id_key:str, caption_key:str):
    ...
    if ext == '.json':
        loaded_data = json.load(open(cap_file))
        
        # If the input follows COCO-style format, read the annotations field.
        if isinstance(loaded_data, dict) and 'annotations' in loaded_data:
            caps = loaded_data['annotations']
        elif isinstance(loaded_data, dict) and 'sentences' in loaded_data:
            caps = loaded_data['sentences']
        elif isinstance(loaded_data, list):
            caps = loaded_data
        else:
            # Fallback for a single-record dict.
            caps = [loaded_data] if isinstance(loaded_data, dict) else loaded_data
    ...
    try:
        ...
        print("Failed to read image ids. Please check the input JSON format.")
        print(f"First record: {caps[0] if caps else 'EMPTY'}")
        raise e
